...
Although Feliz can be used and integrated into your Elmish application inside the `render` functions just any other React binding, The Elm(ish) componentization techniques are known to be adding a lot of boilerplate code to the application structure, especially when it comes to data and event communication between parent and child components. However, Feliz includes a gem called `Feliz.ElmishComponents` that lets you build standalone Elmish components using `init`, `update` and `render` as standalone React components that keep track of their internal state and have their own dispatch loop. Data and event communication will then go through React's best kwown patterns such as using the props.
