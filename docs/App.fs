...
let roughBarChart =
    React.functionComponent (fun () ->
        RoughViz.barChart [ barChart.title "Fruit Sales"
...
let roughHorizontalBarChart =
    React.functionComponent (fun () ->
        RoughViz.horizontalBarChart [ barChart.title "Fruit Sales"
                                      barChart.data fruitSales
                                      barChart.roughness 3
                                      barChart.color color.skyBlue
                                      barChart.stroke color.darkCyan
...
let dynamicRoughChart =
    React.functionComponent (fun () ->
        let (data, setData) =
            React.useStateWithUpdater [ ("point1", 70.0)
                                        ("point2", 40.0)
                                        ("point3", 65.0) ]
        ...
        let addDataPoint () =
            ...
            <| fun previousState ->
                ...
                let nextPoint =
                    (pointLabel, System.Random().NextDouble() * 100.0)
        ...
        Html.div [
                   ...
                   Html.button [ prop.className "button is-primary"
                   ...
                   RoughViz.barChart [ barChart.title title
                                       barChart.data data
...
let samples =
    [ "feliz-elmish-counter", Examples.ElmishCounter.app ()
      ...
      DelayedComponent.render {| load = UseMediaQueryExamples.useResponsiveCustomBreakpointsExample |}
...
let codeBlockRenderer' =
    React.functionComponent (fun (input: {| codeProps: Markdown.ICodeProperties |}) ->
        if input.codeProps.language
           <> null
           && input.codeProps.language.Contains ":" then
            ...
            Html.div [ sampleApp
                       Highlight.highlight [ prop.className "fsharp"
...
let codeBlockRenderer (codeProps: Markdown.ICodeProperties) =
    codeBlockRenderer' {| codeProps = codeProps |}

let renderMarkdown =
    React.functionComponent (fun (input: {| path: string; content: string |}) ->
        Html.div [ prop.className [ Bulma.Content
                                    "scrollbar" ]
                   prop.style [ style.width (length.percent 100)
                                style.padding (0, 20) ]
                   prop.children [ if input.path.StartsWith "https://raw.githubusercontent.com" then
                                       Html.h2 [ Html.i [ prop.className [ FA.Fa; FA.FaGithub ] ]
                                                 Html.anchor [ prop.style [ style.marginLeft 10
                                                                            style.color.lightGreen ]
                                                               prop.href (githubPath input.path)
                                                               prop.text "View on Github" ] ]

                                   Markdown.markdown [ markdown.source input.content
                                                       markdown.escapeHtml false
                                                       markdown.renderers [ markdown.renderers.code codeBlockRenderer ] ] ] ])
...
let nestedMenuList' =
    React.functionComponent (fun (input: {| state: State
                                            name: string
        ...
        Html.li [ Html.anchor [ prop.className Bulma.IsUnselectable
                                prop.onClick
                                <| fun _ ->
                                ...
                                prop.children [ Html.i [ prop.style [ style.marginRight 10 ]
                  ...
                  Html.ul [ prop.className Bulma.MenuList
                            prop.style [ if not collapsed then style.display.none ]

                            prop.children
                                (input.elems
                                 |> List.map (fun f -> f input.basePath)) ] ])

// top level label
let menuLabel' =
    React.functionComponent (fun (input: {| content: string |}) ->
        Html.p [ prop.className [ Bulma.MenuLabel
                                  Bulma.IsUnselectable ]
                 prop.text input.content ])

// top level menu
let menuList' =
    React.functionComponent (fun (input: {| items: Fable.React.ReactElement list |}) ->
        Html.ul [ prop.className Bulma.MenuList
                  prop.style [ style.width (length.percent 95) ]
                  prop.children input.items ])

let menuItem' =
    React.functionComponent (fun (input: {| currentPath: string list
                                            name: string
                                            path: string list |}) ->
        Html.li [ Html.anchor [ prop.className [ if input.currentPath = input.path then Bulma.IsActive
...
let allItems =
    React.functionComponent (fun (input: {| state: State
        ...
        let nestedMenuList (name: string)
                           (basePath: string list)
        ...
        let subNestedMenuList (name: string)
                              (basePath: string list)
                              (items: (string list -> Fable.React.ReactElement) list)
        ...
        Html.div [ prop.className "scrollbar"
                   prop.children [ menuList [ menuItem "Overview" []
                                              ...
                                              nestedMenuList
                                                  ...
                                                  [ nestedMenuItem "Stateless Components" [ Urls.StatelessComponents ]
                                                    ...
                                                    nestedMenuItem
                                                        ...
                                                        [ Urls.EffectfulComponents ]
                                                    nestedMenuItem
                                                        "Subscriptions with Effects"
                                                        [ Urls.SubscriptionsWithEffects ]
                                                  ...
                                                  [ nestedMenuItem "Overview" [ Urls.Overview ]
                                                    ...
                                                    subNestedMenuList
                                                        ...
                                                        [ nestedMenuItem "Simple Line Chart" [ Urls.SimpleLineChart ]
                                                          nestedMenuItem
                                                              ...
                                                              [ Urls.ResponsiveFullWidth ]
                                                          nestedMenuItem
                                                              "Customized Label"
                                                              [ Urls.CustomizedLabelLineChart ]
                                                          nestedMenuItem
                                                              "Optional Values"
                                                              [ Urls.LineChartOptionalValues ]
                                                    ...
                                                    subNestedMenuList
                                                        ...
                                                        [ nestedMenuItem "Simple Area Chart" [ Urls.SimpleAreaChart ]
                                                          ...
                                                          nestedMenuItem
                                                              "Optional Values"
                                                              [ Urls.AreaChartOptionalValues ]
                                                          nestedMenuItem
                                                              "Synchronized Charts"
                                                              [ Urls.SynchronizedAreaChart ]
                                                          nestedMenuItem
                                                              "AreaChartFillByValue"
                                                              [ Urls.AreaChartFillByValue ] ]
                                                    subNestedMenuList
                                                        "Pie Charts"
                                                        [ Urls.PieCharts ]
...
let sidebar =
    React.functionComponent (fun (input: {| state: State
        ...
        // the actual nav bar
        Html.aside [ prop.className Bulma.Menu
                     prop.style [ style.width (length.perc 100) ]
                     prop.children [ menuLabel "Feliz"
                                     allItems
                                         {| state = input.state
                                            dispatch = dispatch |} ] ])
...
let loadOrSegment (origPath: string list) (basePath: string list) (path: string list) =
    ...
    then Html.div [ for segment in origPath -> Html.p segment ]
...
let main =
    React.functionComponent (fun (input: {| state: State
        ...
        Html.div [ prop.className [ Bulma.Tile
                                    Bulma.IsAncestor ]
                   prop.children [ Html.div [ prop.className [ Bulma.Tile; Bulma.Is2 ]
...
let render' =
    React.functionComponent (fun (input: {| state: State
        ...
        let application =
            Html.div [ prop.style [ style.padding 30 ]
                       prop.children [ main
                                           {| state = input.state
                                              dispatch = dispatch |} ] ]
