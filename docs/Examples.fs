...
let keyedFragments state dispatch =
    React.keyedFragment
        (1,
         [ Html.div [ React.keyedFragment
                          ("hello",
                           [ Html.h1 "Hello"
                             Html.div []
                             Html.div [ Html.h1 "More stuff" ]
                             Html.div [ Html.h2 [ Html.strong "Bold" ] ]
                             Html.ul [ Html.li [ Html.strong "Wow" ]
                                       Html.li "So"
                                       Html.li (Html.em "Lightweight")
                                       Html.ol [ Html.li [ Html.strong "More" ]
                                                 Html.li []
                                                 Html.li [ Html.none ] ] ] ]) ] ])

let inputs () =
    Html.form [ Html.input [ prop.type'.checkbox ]
                Html.input [ prop.type' "password" ]
                Html.input [ prop.type'.date
                             prop.min DateTime.Now ] ]

let staticHtml =
    React.functionComponent (fun () ->
        let html =
            Html.div [ prop.style [ style.padding 20 ]
                       prop.children [ Html.h1 "Html content"
                                       Html.br [] ] ]


        Html.pre [ Html.text (ReactDOMServer.renderToString html) ])
...
let staticMarkup =
    React.functionComponent (fun () ->
        ...
        Html.pre [ Html.text (ReactDOMServer.renderToStaticMarkup html) ])
...
let multipleStateVariables =
    React.functionComponent (fun () ->
        ...
        Html.div [ Html.h1 [ prop.style [ style.color textColor ]
                             prop.text count ]

                   Html.button [ prop.text "Increment"
                                 prop.onClick (fun _ -> setCount (count + 1)) ]

                   Html.button [ prop.text "Red"
                                 prop.onClick (fun _ -> setTextColor (color.red)) ]

                   Html.button [ prop.text "Blue"
...
let asyncEffect =
    React.functionComponent (fun () ->
        ...
        Html.div [ if isLoading then Html.h1 "Loading" else Html.h1 content ])
...
let timer =
    React.functionComponent (fun () ->
        ...
        let subscribeToTimer () =
            // start the ticking
            let subscriptionId =
                setTimeout (fun _ -> if not paused then setValue (value + 1)) 1000
        ...
        Html.div [ Html.h1 value
                   ...
                   Html.button [ prop.className [ Bulma.Button
                                                  Bulma.IsLarge
...
let effectfulUserId =
    React.functionComponent (fun () ->
        ...
        Html.div [ Html.h1 [ prop.style [ style.color textColor ]
                   ...
                   Html.button [ prop.text "Update User ID"
                                 prop.onClick (fun _ -> setUserId (rnd.Next(1, 100))) ] ])

let effectfulTabCounter =
    React.functionComponent (fun () ->
        let (count, setCount) = React.useState (0)
...
let counterApp state dispatch =
    Html.div [ prop.id "main"
               ...
               prop.children [
                               ...
                               Html.button [ prop.style [ style.marginRight 5 ]
...
let fragmentTests =
    Html.div [ prop.children [ React.fragment [ Html.h1 "One"
                                                Html.h2 "Two" ] ] ]

let animationsOnHover' =
    React.functionComponent (fun (props: {| content: ReactElement |}) ->
        [ let (hovered, setHovered) = React.useState (false)
          Html.div [ prop.style [ style.padding 10
                                  style.transitionDuration (TimeSpan.FromMilliseconds 1000.0)
                                  style.transitionProperty [ transitionProperty.backgroundColor
...
let animationsOnHover content =
    animationsOnHover' {| content = React.fragment content |}

let animationSample =
    Html.div [ animationsOnHover [ Html.span "Hover me!" ]
...
let portalsPopup =
    React.functionComponent (fun () ->
        Html.div [ prop.style [ style.position.absolute
                   ...
                   prop.children [ Html.p [ prop.text "Portals can be used to escape the parent component." ] ] ])
...
let focusInputExample =
    React.functionComponent (fun () ->
        ...
        Html.div [ Html.input [ prop.style [ style.caretColor.red ]
                                prop.ref inputRef
                                prop.type'.text ]

                   Html.button [ prop.onClick (fun _ -> focusTextInput ())
                                 prop.text "Focus Input" ] ])

let forwardRefChild =
    React.forwardRef (fun ((), ref) ->
        Html.input [ prop.type'.text
                     prop.ref ref ])
...
let forwardRefParent =
    React.functionComponent (fun () ->
        ...
        Html.div [ forwardRefChild ((), inputRef)
                   Html.button [ prop.text "Focus Input"
                                 prop.onClick
                                 <| fun ev ->
                                     inputRef.current
                                     |> Option.iter (fun elem -> elem.focus ()) ] ])

let forwardRefImperativeChild =
    React.forwardRef (fun ((), ref) ->
        ...
        React.useImperativeHandle
            (ref,
             (fun () ->
                 inputRef.current
                 |> Option.map (fun innerRef -> {| focus = fun () -> setDivText innerRef.className |})))

        Html.div [ Html.input [ prop.className "Howdy!"
                                prop.type'.text
                                prop.ref inputRef ]
                   Html.div [ prop.text divText ] ])

let forwardRefImperativeParent =
    React.functionComponent (fun () ->
        let ref =
            React.useRef<{| focus: unit -> unit |} option> (None)

        Html.div [ forwardRefImperativeChild ((), ref)
                   Html.button [ prop.text "Focus Input"
                                 prop.onClick
                                 <| fun ev ->
                                     ref.current
...
type StrictModeWarning() =
    ...
    override _.render() =
        Html.div [ prop.text "I cause a warning!" ]

let strictModeExample =
    React.functionComponent (fun () ->
        Html.div [ prop.style [ style.display.inheritFromParent ]
                   prop.children [ React.strictMode [ Fable.React.Helpers.ofType<StrictModeWarning, obj, obj> "" [] ] ] ])

let myNonCodeSplitComponent =
    React.functionComponent (fun () -> Html.div [ prop.text "I was loaded synchronously!" ])
...
let centeredSpinner =
    Html.div [ prop.style [ style.textAlign.center
                            ...
                            style.marginTop 50 ]
               prop.children [ Html.li [ prop.className [ FA.Fa
                                                          FA.FaRefresh
                                                          FA.FaSpin
                                                          FA.Fa3X ] ] ] ]

let asyncComponent: JS.Promise<unit -> ReactElement> =
    JsInterop.importDynamic "./CodeSplitting.fs"

let codeSplitting =
    React.functionComponent (fun () ->
        Html.div [ prop.children [ myNonCodeSplitComponent ()
                                   React.suspense ([ Html.div [ React.lazy' (asyncComponent, ()) ] ], centeredSpinner) ] ])

let codeSplittingDelayed =
    React.functionComponent (fun () ->
        Html.div [ prop.children [ myNonCodeSplitComponent ()
                                   React.suspense
                                       ([ Html.div [ React.lazy'
...
let useStateNormal =
    React.forwardRef (fun ((), ref) ->
        ...
        Html.div [ prop.classes [ Bulma.Box ]
                   prop.children [ Html.div [ prop.text (sprintf "Normal Count: %i" count) ] ] ])

let useStateLazy =
    React.forwardRef (fun ((), ref) ->
        let count, setCount =
            React.useState (fun () -> sortNumbers ())
        ...
        Html.div [ prop.classes [ Bulma.Box ]
                   prop.children [ Html.div [ prop.text (sprintf "Lazy Count: %i" count) ] ] ])

let useStateNormalVsLazy =
    React.functionComponent (fun () ->
        ...
        Html.div [ prop.className Bulma.Control
                   prop.style [ style.paddingLeft (length.em 8)
                                style.paddingBottom (length.em 1) ]
                   prop.children [ Html.div [ prop.style [ style.maxWidth (length.em 15)
                                                           style.paddingBottom (length.em 2) ]
                                              prop.children [ Html.div [ prop.style [ style.textAlign.center
                                                                                      style.marginLeft length.auto
                                                                                      style.marginRight length.auto
                                                                                      style.marginTop 50
                                                                                      style.paddingBottom (length.em 2) ]
                                   ...
                                   Html.button [ prop.classes [ Bulma.Button
                                                 ...
                                                 prop.onClick
                                                 <| fun _ ->
                                                     rerenderNormal.current
                                                     |> Option.iter (fun f -> f (rng.Next(0, 10)))
                                   ...
                                   Html.button [ prop.classes [ Bulma.Button
                                                 ...
                                                 prop.onClick
                                                 <| fun _ ->
                                                     rerenderLazy.current
                                                     |> Option.iter (fun f -> f (rng.Next(0, 10)))
                                                 prop.text "Rerender lazy" ] ] ])

module TokenCancellation =
    let useToken =
        React.functionComponent (fun (input: {| failedCallback: unit -> unit |}) ->
    ...
    let result =
        React.functionComponent (fun (input: {| text: string |}) -> Html.div input.text)
    ...
    let render =
        React.functionComponent (fun () ->
            ...
            let setFailed =
                ...
                <| fun () -> setResultText "You didn't cancel me in time!"

            Html.div [ if renderChild
                       then useToken {| failedCallback = setFailed |}
                       ...
                       Html.button [ prop.classes [ Bulma.Button
                                                    ...
                                                    Bulma.HasTextWhite ]
                                     prop.disabled
                                     <| (renderChild && resultText = "Pending...")
                                     prop.onClick
                                     <| fun _ ->
                                         async {
